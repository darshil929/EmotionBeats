...
class SpotifyClient:
    ...
    async def for_user(cls, db: Session, user_id: str) -> "SpotifyClient":
        ...
        if user.spotify_token_expiry and user.spotify_token_expiry <= datetime.utcnow():
            if not user.spotify_refresh_token:
                raise ValueError("Refresh token not available")
            
            # Refresh the token
            token_data = await SpotifyAuthService.refresh_token(user.spotify_refresh_token)
            
            # Update user record
            user.spotify_access_token = token_data.access_token
            # Add expires_in seconds to current time
            user.spotify_token_expiry = datetime.utcnow() + timedelta(seconds=token_data.expires_in)
    ...
    async def search_tracks(
        ...
        return [SpotifyTrack(**item) for item in data.get("tracks", {}).get("items", [])]
