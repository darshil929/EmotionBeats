...
class TestUserCRUD:
    ...
    def test_user_read(self, test_user, db_session):
        ...
        by_username = db_session.query(User).filter(User.username == test_user.username).first()
...
class TestPreferencesCRUD:
    ...
    def test_preferences_create(self, test_user, db_session):
        ...
        retrieved = db_session.query(Preferences).filter(
    ...
    def test_preferences_read(self, test_user, db_session):
        ...
        # Read by ID
        retrieved = db_session.query(Preferences).filter(
            Preferences.id == prefs_id
        ).first()
        assert retrieved is not None
        assert "pop" in retrieved.preferred_genres
        
        # Read by user_id
        by_user = db_session.query(Preferences).filter(
            Preferences.user_id == test_user.id
        ).first()
...
class TestChatSessionCRUD:
    ...
    def test_chat_session_create(self, test_user, db_session):
        ...
        retrieved = db_session.query(ChatSession).filter(
            ChatSession.session_identifier == "test_session_1"
        ).first()
        assert retrieved is not None
...
class TestPlaylistCRUD:
    ...
    def test_playlist_create(self, test_user, chat_session, db_session):
        ...
        retrieved = db_session.query(Playlist).filter(
            Playlist.spotify_playlist_id == "sp123"
        ).first()
        assert retrieved is not None
